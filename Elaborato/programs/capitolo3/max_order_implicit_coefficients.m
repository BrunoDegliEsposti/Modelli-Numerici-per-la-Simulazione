function [alpha_exact,beta_exact] = max_order_implicit_coefficients(k)
%MAX_ORDER_IMPLICIT_COEFFICIENTS Calcola in aritmetica esatta i coefficienti
% alpha e beta dell'unico metodo lineare a k passi implicito di ordine 2k.
...
% La variabile n tiene traccia del numero del vincolo che si va a imporre.
    ...
    A(n,1:k+1) = 1;
    n = n+1;
    % imponi che il metodo abbia ordine 2k
    for j = 1:2*k
        for i = 0:k
            A(n,i+1) = i^j;
            A(n,k+1+i+1) = -j*(i^(j-1));
